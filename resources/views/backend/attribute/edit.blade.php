            ...
            <div class="col-lg-12">
                <div class="card card-primary card-outline">
                    <div class="card-header">
                        <h5 class="m-0" style="float: left">Edit Attribute</h5>
                        <a href="{{ route('admin.attribute.index') }}" class="btn btn-success" style="float: right">Go
                            Back</a>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('admin.attribute.update',$proAttr->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="form-group">
                                <label for="">Attribute Name</label>
                                <input type="text" name="attr_name" value="{{old('attr_name',$proAttr->attr_name)}}"
                            ...
                            <label>
                                <input type="checkbox" name="cats_id[]" value="{{ $cat->id }}"
                                    {{ in_array($cat->id, $proAttr->cats_id) ? ' checked' : '' }}>
                                {{ $cat->title }}
                            </label>
